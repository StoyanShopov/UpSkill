...
export const getFilteredCoaches = async (filterArr) => {
  let allCoaches = await getAllCoaches();
  let filteredCoaches = [];
  ...
  for (let i = 0; i < filterArr.length; i++) {
    allCoaches
      .filter((coach) => coach.coachField.includes(filterArr[i]))
      .map((fc) => filteredCoaches.push(fc));
  }
